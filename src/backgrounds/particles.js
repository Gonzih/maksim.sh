/**
 * Particle field background module.
 *
 * d3 force-directed topology: nodes cycle through four spatial arrangements
 * (constellation, monogram, signal grid, golden-angle flow) wired together by
 * Delaunay adjacency, drawn to a canvas.
 */
...
/** Uncapped: the trail fade depends on running at the display refresh rate. */
export const frameInterval = 0;

/** How long a click shockwave takes to cross and settle, in milliseconds. */
const BURST_LIFE = 1_800;
...
export function create(host, options = {}) {
  ...
  function updatePhysics(elapsed) {
    ...
    for (const node of nodes) {
      ...
      // A click sends an expanding pressure front through the graph. Nodes
      // get a real outward impulse as it passes; the existing target forces
      // are what pull them back, so they drift home instead of snapping.
      for (const burst of bursts) {
        const age = (elapsed - burst.startedAt) / BURST_LIFE;
        if (age >= 1) continue;

        const deltaX = node.x - burst.x;
        const deltaY = node.y - burst.y;
        const distance = Math.hypot(deltaX, deltaY) || 1e-4;
        const front = easeCubicInOut(age) * Math.min(width, height) * 0.62;
        const band = Math.max(0, 1 - Math.abs(distance - front) / 110);
        if (band <= 0.001) continue;

        const impulse = band * (1 - age) ** 1.5 * 2.4;
  ...
  function drawBursts(elapsed) {
    for (let index = bursts.length - 1; index >= 0; index -= 1) {
      ...
      const progress = (elapsed - burst.startedAt) / BURST_LIFE;
      if (progress >= 1) {
      ...
      context.globalCompositeOperation = "screen";
      context.beginPath();
      context.arc(burst.x, burst.y, easeCubicInOut(progress) * Math.min(width, height) * 0.62, 0, TAU);
  ...
  return {
    ...
    addRipple(clientX, clientY, elapsed) {
      ...
      bursts.push({
        ...
        y: clientY,
        startedAt: elapsed,
        color: Math.floor((clientX / Math.max(1, width)) * PALETTE.length),
      });
    },
    setOpacity(value) {
